...
Q^*(s,a) = \max_{\pi} \mathbb{E}_{\pi}\left[G_t | s_t = s, a_t = a\right].
\end{gather*}
...
\begin{gather}
     V^{\pi}(s) = \sum_a \pi(a \mid s) \left(R(s,a) + \gamma \sum_{s'} p(s' \mid s,a) V^{\pi}(s') \right) \label{expanded_v_bell},\\
      Q^{\pi}(s,a) = R(s,a) + \gamma \sum_{s'} p(s' \mid s,a) \sum_{a'} \pi(a' \mid s') Q^{\pi}(s',a')  \label{expanded_q_bell}.\footnotemark
\end{gather}\footnotetext{Since $ \sum_{a'} \pi(a' \mid s') Q^{\pi}(s',a') = \mathbb{E}_{a'\sim \pi}[Q^{\pi}(s',a')] = V^{\pi}(s') \rightarrow Q^{\pi}(s,a) =  R(s,a) + \gamma \sum_{s'} p(s' \mid s,a)  V(s') $}
