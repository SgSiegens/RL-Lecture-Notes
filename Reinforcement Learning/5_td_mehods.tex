...
the discounted return starting from state $s$ at time $t$. The core idea of Monte 
...
\begin{figure}[H]
    \centering
     \includegraphics[width=\linewidth]{images/TD_MC_DP_backups.png}
    \caption{Visualization of Monte Carlo methods (left), Temporal Difference (TD) learning (middle), and Dynamic Programming (right) in a backup diagram.
The red highlights indicate how each method computes value functions: Monte Carlo uses the entire episode, TD learning performs a one-step lookahead using bootstrapping, and Dynamic Programming computes exact expectations but only when the transition probabilities are known.}
  \end{figure}
  ...
  We have thus far considered two extremes for policy evaluation. On one hand, Monte Carlo methods use the full,
  unbiased episodic return ($G_t$), but they suffer from high variance because the estimate depends on the entire
  sequence of stochastic transitions and rewards until the episode's end. On the other hand, one-step Temporal-
  Difference learning (TD(0)) uses a low-variance estimate by bootstrapping after a single step 
  ($R_{t+1} + \gamma V(S_{t+1})$), but this introduces significant bias from its reliance on the current (and
  likely imperfect) value estimate.\newline To find a middle ground and control this bias-variance trade-off,
  we can use \textit{n-step returns}. The core idea is to look $n$ steps into the future, accumulating actual rewards,
  and only then bootstrap using the value estimate of the resulting state. The \textbf{$n$-step return} at time $t$, 
  denoted $G_t^{(n)}$, is formally defined as:
  $$
  G_t^{(n)} = R_{t+1} + \gamma R_{t+2} + \dots + \gamma^{n-1} R_{t+n} + \gamma^n V(S_{t+n})
  $$
  where $V(S_{t+n})$ is the current estimate of the value of state $S_{t+n}$. If the episode terminates before 
  $t+n$, the return is simply the full Monte Carlo return $G_t$, and $V(S_{\text{terminal}}) \equiv 0$. The 
  corresponding $n$-step TD update for the state-value function $V(S_t)$ is then:
  $$
  V(S_t) \leftarrow V(S_t) + \alpha \left[ G_t^{(n)} - V(S_t) \right]
  $$
  This formulation elegantly unifies MC and TD(0). Note that for $n=1$, we recover the TD(0) update, as 
  $G_t^{(1)} = R_{t+1} + \gamma V(S_{t+1})$. As $n \to \infty$ (or $n$ exceeds the episode length), $G_t^{(n)}$ 
  becomes the standard Monte Carlo return $G_t$. The parameter $n$ is now a hyperparameter. Rather than selecting
  a single "best" $n$, we can combine all possible $n$-step returns. This leads to the \textbf{$\lambda$-return},
  $G_t^\lambda$, which creates a compound return by averaging all $n$-step returns, weighting each by 
  $(1-\lambda)\lambda^{n-1}$. This is the foundation of the TD($\lambda$) algorithm.
  \paragraph{Eligibility traces}
  However, a practical problem persists, which we first encountered with Monte Carlo methods. To compute the 
  $n$-step return $G_t^{(n)}$ (or the full $\lambda$-return), we must wait until time $t+n$ (or the end of the 
  episode for $G_t^\lambda$) to observe the required rewards and the state $S_{t+n}$. Only then can we compute 
  the update for $V(S_t)$. This delay is undesirable, as it prevents the algorithm from being fully online. 
  This "forward-view" perspective motivates the need for an equivalent "backward-view" mechanism, 
  which is provided by eligibility traces.\newline To solve the problem of update delays, we introduce
  \textit{eligibility traces}. This "backward-view" mechanism provides a computationally efficient and
  fully online method that is mathematically equivalent to the "forward-view" $\lambda$-return. The intuition is 
  as follows: an eligibility trace is a short-term memory that tracks which states have been visited recently.
  When a "surprise" occurs (i.e., the TD error is high, meaning our prediction was wrong), we assign credit or
  blame for this surprise not just to the immediately preceding state, but to all recently visited states, 
  proportional to their eligibility.\newline  Mechanically, we maintain an eligibility trace, $E_t(s)$, for every
  state $s$. At the beginning of an episode, all traces are zero. At each time step $t$, the trace for the visited
  state $S_t$ is incremented, and all other traces decay:
  $$
  E_t(s) =
  \begin{cases}
  ...
  \end{cases}
  $$
  This is known as an *accumulating trace*. The parameter $\lambda \in [0, 1]$ controls the rate of decay:
  \begin{itemize}
    \item If $\lambda=0$, $E_t(s)$ is 1 only for the current state $s=S_t$ and 0 otherwise. This reduces to the standard one-step TD(0) update.
    \item If $\lambda=1$, the trace does not decay (within an episode), which relates the algorithm to Monte Carlo methods.
  \end{itemize}

  With eligibility traces, the update process becomes simple and online. At time $t+1$, we observe the reward $R_{t+1}$ and the next state $S_{t+1}$. We then compute the one-step TD error:
  $$
  \delta_t = R_{t+1} + \gamma V(S_{t+1}) - V(S_t)
  $$
  This single error signal $\delta_t$ is then used to update \textit{all} states in the environment, weighted by their current eligibility trace:
  $$
  V(s) \leftarrow V(s) + \alpha \delta_t E_t(s), \quad \text{for all } s \in \mathcal{S}
  $$
  This backward-view update, performed at every step, has been proven to be equivalent to the forward-view
  TD($\lambda$) algorithm. It achieves the same effect as averaging all $n$-step returns, but does so efficiently,
  without waiting for future time steps.
  \newline 
  ...
  Recall that the advantage function is defined as $A^\pi(s, a) = Q^\pi(s, a) - V^\pi(s)$. The key idea behind the
  \textit{Dueling DQN} architecture \cite{wang2016duelingnetworkarchitecturesdeep} is to separately estimate two components: the \textbf{state-value function}
  \(V(s)\) and the \textbf{advantage function} \(A(s, a)\). These two outputs are then combined to compute the 
  action-value function \(Q(s, a)\). This decoupling allows the network to independently learn the value of a
  given state and the relative advantages of actions within that state. An important detail is that when computing 
  the advantages, we ensure they have zero mean across all actions in order to be complient with the definition of the 
  advantge function:
  \[
    A(s, a) \leftarrow A(s, a) - \frac{1}{|\mathcal{A}|} \sum_{a'} A(s, a')
  \]
  Overall, the Dueling DQN architecture leads to more stable and faster convergence, particularly in environments where 
