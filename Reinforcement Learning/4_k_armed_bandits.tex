...
entropy $H$ is a good measure (exploration). Entropy is a concept from information theory 
that measures the randomness or uncertainty of a probability distribution, which, in our case, is the stochastic policy $\pi(a|s)$.
A policy with High Entropy means the action probabilities are nearly equal for all actions. This signifies high uncertainty on the 
agent's part, as its action choices are very random. This state promotes greater exploration because the agent is not strongly 
committed to any single action.\newline Conversely, a policy with Low Entropy means the agent assigns a high probability to one or a few actions 
(approaching a deterministic policy). This signifies high confidence in those actions, as the agent's choices are predictable. This state promotes 
greater exploitation as the agent focuses on the action it believes to be optimal. The new the objective which includes this idea of entropy looks like this 
\begin{align*}
